...
function RoomView({ room, onBack }: { room: Room; onBack: () => void }) {
  ...
  const [draftAgent, setDraftAgent] = useState<Pick<Agent, "name" | "role" | "goal">>({
    name: "",
    role: "",
    goal: "",
  });
  ...
  async function addAgent() {
    ...
    try {
      ...
      const agent = await api<Agent>("/agents", { method: "POST", body: JSON.stringify(payload) });
    ...
    <div style={{ maxWidth: 1000, margin: "2rem auto", padding: "0 1rem" }}>
      ...
      <div style={{ border: "1px solid #ddd", borderRadius: 8, padding: 12, marginBottom: 16 }}>
        ...
        <div style={{ display: "grid", gap: 8, gridTemplateColumns: "repeat(3, 1fr)", marginBottom: 8 }}>
          <input
            ...
            onChange={(e) => setDraftAgent((s) => ({ ...s, name: e.target.value }))}
            style={{ padding: 8 }}
          />
          <input
            placeholder="Role"
            value={draftAgent.role}
            onChange={(e) => setDraftAgent((s) => ({ ...s, role: e.target.value }))}
            style={{ padding: 8 }}
          />
          <input
            placeholder="Goal"
            value={draftAgent.goal}
            onChange={(e) => setDraftAgent((s) => ({ ...s, goal: e.target.value }))}
            style={{ padding: 8 }}
          />
        </div>
        <button onClick={addAgent} disabled={loading || !draftAgent.name || !draftAgent.role} style={{ padding: "8px 12px" }}>
          Add Agent
        </button>
        <div style={{ marginTop: 8, color: "#666" }}>
          {agents.length ? `Agents: ${agents.map((a) => a.name).join(", ")}` : "No agents yet."}
        </div>
      </div>
      ...
      <div style={{ border: "1px solid #ddd", borderRadius: 8, padding: 12 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
          ...
          <button onClick={simulateTurn} disabled={loading || agents.length === 0} style={{ padding: "8px 12px" }}>
            {loading ? "Working..." : "Simulate Turn"}
          </button>
        </div>
        <div style={{ border: "1px solid #eee", borderRadius: 6, padding: 8, height: 420, overflow: "auto" }}>
          {messages.map((m) => (
...
export default function App() {
  ...
  return open ? <RoomView room={open} onBack={() => setOpen(null)} /> : <RoomsView onOpen={setOpen} />;
}
